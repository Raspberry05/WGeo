    ...
    <div style={{
      ...
      {list.map((ac) => {
        ...
        return (
          ...
          >
            <div>
              <div style={{ color: isSelected ? color : '#ccddcc', fontWeight: 'bold' }}>
                {ac.callsign}
              </div>
              <div style={{ color: '#4a6a5a', fontSize: '9px', marginTop: '1px' }}>
                {ac.status.toUpperCase()}
              </div>
            </div>
            <div style={{ textAlign: 'right', color: '#7a9a8a' }}>
              <div>{Math.round(ac.altitude).toLocaleString()}ft</div>
              <div style={{ fontSize: '9px', color: '#4a6a5a' }}>{Math.round(ac.velocity)}kts</div>
            </div>
          </div>
        );
      })}
