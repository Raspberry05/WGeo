...
export function AircraftInspector() {
  ...
  const STATUS_COLORS: Record<string, string> = {
    airborne: '#00ff88', landing: '#ffaa00', taxiing: '#00aaff', parked: '#666666',
  };
  const color = STATUS_COLORS[ac.status] ?? '#ffffff';

  const Field = ({ label, value }: { label: string; value: string | number }) => (
    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '4px 0', borderBottom: '1px solid #0d1f10' }}>
      <span style={{ color: '#4a6a5a', fontSize: '10px' }}>{label}</span>
      <span style={{ color: '#ccddcc', fontFamily: 'monospace', fontSize: '11px' }}>{value}</span>
    </div>
  );
  ...
  return (
    <div style={{
      ...
      <div style={{ marginBottom: '8px' }}>
        <div style={{ color, fontSize: '14px', fontWeight: 'bold', letterSpacing: '1px' }}>
          {ac.callsign}
        </div>
        <div style={{ color: '#4a6a5a', fontSize: '9px' }}>{ac.icao24.toUpperCase()}</div>
      </div>

      <Field label="STATUS" value={ac.status.toUpperCase()} />
      <Field label="ALTITUDE" value={`${Math.round(ac.altitude).toLocaleString()} ft`} />
